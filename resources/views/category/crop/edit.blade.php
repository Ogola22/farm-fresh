    ...
    <div class="container container-contact100">
        <div class="wrap-contact100">
            <form method="POST" action="{{route('crop.update',$crop->id)}}" class="contact100-form validate-form">
                @csrf
                @method('PUT')
                <span class="contact100-form-title">crop Details</span>

                <label for="name" class="contact100-form-label">{{_('name')}}</label>
                <div class="wrap-input100 validate-input" data-validate="Please enter your name">
                    <input class="form-control input100 @error('name') is-invalid @enderror" id="name" type="text" name="name" value="{{$crop->name}}" placeholder="Enter crop's breed" autocomplete="name" autofocus required>
                    @error('name')
                ...
                <div class="wrap-input100 validate-input " data-validate="Crop Duration in Number of Days">
                    <input class="form-control input100 @error('quantity') is-invalid @enderror" id="quantity" type="number" name="quantity" value="{{$crop->quantity}}" placeholder="The number of crops you have" autocomplete="quantity" autofocus required>
                    @error('quantity')
                    <span class="focus-input100 invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
